...
int AlsaPcm::set_hwparams(snd_pcm_t* handle, snd_pcm_hw_params_t* params) {
    ...
    if (err < 0) {
        throw OstrostrojException(fmt::format("APCM  rate {}Hz not available for playback: {}", PCM_OUT_RATE, snd_strerror(err)));
    }
    if (rrate != PCM_OUT_RATE) {
        throw OstrostrojException(fmt::format("APCM  rate doesn't match (requested {}Hz, get {}Hz)", PCM_OUT_RATE, err));
    }
