...
MidiEncoder::MidiEncoder(uint8_t _channel, uint _param, int _min_value, int _max_value):
    ...
    if (min_value >= max_value) {
        throw OstrostrojException(fmt::format("ENCDR invalid min max [min={},max={}]", min_value, max_value));
    }
}
...
bool MidiEncoder::handle(uint8_t _channel, uint _param, int _value) {
    ...
    if (value == INT_MIN || _value == value) {
        value = _value;
        grabbed = true;
        SPDLOG_DEBUG("ENCDR grabbed init[value={}]", value);
        return true;
    }
    ...
    if (start <= value && value <= end) {
        grabbed = true;
        value = _value;
        SPDLOG_DEBUG("ENCDR grabbed[value={}]", value);
        return true;
    }
    prev_value = _value;
    return true;
