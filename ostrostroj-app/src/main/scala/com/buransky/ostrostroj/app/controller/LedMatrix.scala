  ...
  def apply(driver: ActorRef[DriverCommand], config: Config): Behavior[LedMatrixCommand] = Behaviors.setup { ctx =>
    ...
    Behaviors.receiveMessage {
      ...
      case Test =>
        ...
        sendToAllChips(oldMax7219, driver, 0x0C, 0x00)
        sendToAllChips(oldMax7219, driver, 0x0C, 0x01)

        // Turn off test mode
        sendToAllChips(oldMax7219, driver, 0x0F, 0x01)
        sendToAllChips(oldMax7219, driver, 0x0F, 0x00)
        // Set decode mode (no decoding)
        sendToAllChips(oldMax7219, driver, 0x09, 0x00)
        // Set scan limit (8 segments)
        sendToAllChips(oldMax7219, driver, 0x0B, 0x07)
        // Set min intensity
        sendToAllChips(oldMax7219, driver, 0x0A, 0x00)
...
//        max7219 ! Max7219.Word(0x08, 0x04, 3)
  ...
  private def sendToAllChips(oldMax7219: ActorRef[OldMax7219.Command],
                             driver: ActorRef[DriverCommand], address: Byte, data: Byte): Unit = {
    for (i <- 0 to 3) {
      // Old way:
